...
# author: ynie
# date: April, 2020
from net_utils.utils import load_device
...
from pose_detect.Model.total3d import TOTAL3D
...
def load_demo_data(demo_path, device):
    ...
    rel_pair_counts = torch.cat([
        torch.tensor([0]),
        torch.cumsum(
            torch.pow(data['obj_split'][:, 1] - data['obj_split'][:, 0], 2), 0)
    ], 0)
    cls_codes = torch.zeros([size_cls.size(0), 9]).to(device)
    cls_codes[range(size_cls.size(0)), [
        NYU37_TO_PIX3D_CLS_MAPPING[cls.item()]
        for cls in torch.argmax(size_cls, dim=1)
    ]] = 1
    ...
    input_data = {
        ...
        'size_cls': size_cls,
        'split': split,
        'rel_pair_counts': rel_pair_counts,
        'cls_codes': cls_codes,
        'bdb2D_pos': bdb2D_pos
    }
    return input_data
...
def run(cfg):
    ...
    cam_R_out = get_rotation_matix_result(
        cfg.bins_tensor, torch.argmax(est_data['pitch_cls_result'],
                                      1), est_data['pitch_reg_result'],
        torch.argmax(est_data['roll_cls_result'], 1),
    ...
    # save results
    nyu40class_ids = [
        int(evaluate_bdb['classid']) for evaluate_bdb in bdb3D_out_form_cpu
    ]
    ...
    for obj_id, obj_cls in enumerate(current_cls):
        ...
        mesh_obj = {
            'v': current_coordinates[obj_id].transpose(-1, -2).cpu().numpy(),
    ...
    pre_layout_data = sio.loadmat(os.path.join(save_path,
                                               'layout.mat'))['layout']
    ...
    vtk_objects, pre_boxes = format_mesh(
        glob(os.path.join(save_path, '*.obj')), pre_boxes)

    image = np.array(
        Image.open(os.path.join(cfg.config['demo_path'],
                                'img.jpg')).convert('RGB'))
    ...
    scene_box.draw_projected_bdb3d('prediction',
                                   if_save=True,
                                   save_path='%s/3dbbox.png' % (save_path))
    # if_save : True -> save results; False -> render results
