...
from pose_detect.Loss.recon_loss import ReconLoss
    ...
    def get_metric_values(self, est_data, gt_data):
        ...
        if 'lo_ori_reg_result' in est_data.keys():
            ...
            cam_R_out, pitch, roll = get_rotation_matix_result(
                ...
                torch.argmax(est_data['pitch_cls_result'], 1),
                est_data['pitch_reg_result'],
                torch.argmax(est_data['roll_cls_result'], 1),
                est_data['roll_reg_result'],
                return_degrees=True)
            cam_pitch_err = torch.abs(pitch -
                                      gt_data['cam_pitch_gt']).mean().item()
            cam_roll_err = torch.abs(roll -
                                     gt_data['cam_roll_gt']).mean().item()
        ...
        if self.cfg.config['full'] and 'meshes' in est_data.keys():
            ...
            if isinstance(est_data['meshes'], list):
                mesh_data['meshes'] = [
                    est_data['meshes'][i]
                    for i in gt_data['mask_status'].nonzero()[0]
                ]
            else:
                mesh_data['meshes'] = est_data['meshes'][
                    gt_data['mask_status'].nonzero()]
            mesh_loss = self.mesh_reconstruction_loss(mesh_data, gt_data, {
                'cam_R_result': cam_R_out,
                'bdb3D_result': bdb3D_out
            })
            metrics['Lg'] = getattr(mesh_loss['mesh_loss'], 'item',
                                    lambda: mesh_loss['mesh_loss'])()
        '''Save results'''
        if self.cfg.config['log']['save_results'] \
            ...
            for index, sequence_id in enumerate(gt_data['sequence_id']):
                ...
                if 'world_R_inv' in gt_data:
                    ...
                    bdb3d_inv = gt_data['bdb3d_inv'][
                        interval[0]:interval[1]].cpu().numpy()
                    layout_3D = get_layout_bdb_from_corners(
                        lo_bdb3D_out[index, :, :].cpu().numpy())
                    ...
                    layout_3D_co['basis'] = np.matmul(lo_inv,
                                                      layout_3D_co['basis'])
                    layout_3D_co['coeffs'] = np.matmul(lo_inv,
                                                       layout_3D_co['coeffs'])
                    layout_3D_co = get_corners_of_bb3d_no_index(
                        layout_3D_co['basis'], layout_3D_co['coeffs'],
                        layout_3D_co['centroid'])
                    trans_mat = np.array([[0., 0., 1.], [1., 0., 0.],
                                          [0., 1., 0.]])
                ...
                if est_data['meshes'] is not None:
                    ...
                    current_coordinates = est_data['meshes'][
                        interval[0]:interval[1]]

                    for obj_id, obj_cls in enumerate(current_cls):
                        file_path = os.path.join(
                            save_path_per_img, '%s_%s.obj' % (obj_id, obj_cls))

                        mesh_obj = {
                            'v':
                            current_coordinates[obj_id].transpose(
                                -1, -2).cpu().numpy(),
                            'f':
                            current_faces[obj_id].cpu().numpy()
                        }
