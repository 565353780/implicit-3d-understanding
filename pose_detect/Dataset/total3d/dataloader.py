...
from pose_detect.Dataset.total3d.dataset import Total3D_Dataset
...
def Total3D_collate_fn(batch):
        ...
        if key == 'boxes_batch':
            ...
            for subkey in batch[0][key]:
                if subkey == 'mask':
                    ...
                    list_of_tensor = [
                        recursive_convert_to_torch(elem[key][subkey])
                        for elem in batch
                    ]
    ...
    collated_batch['obj_split'] = torch.tensor(
        [[sum(interval_list[:i]),
          sum(interval_list[:i + 1])] for i in range(len(interval_list))])
