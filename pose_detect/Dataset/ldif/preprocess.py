...
from pose_detect.Config.configs import PIX3DConfig, ShapeNetConfig

from pose_detect.Method.preprocess import normalize, remove_if_exists
...
class PreProcesser(object):
    ...
    def make_output_folder(self, mesh_path):
        ...
        output_mesh_folder_name = mesh_unit_path.split(mesh_class_name +
                                                       "/")[1].replace(
                                                           ".obj", "").replace(
                                                               "/", ".")
    ...
    def processImage(self, sample):
        output_folder = self.make_output_folder(self.config.metadata_path +
                                                sample['model'])
        ...
        if not self.skip_done or not os.path.exists(output_path):
            img = np.array(
                Image.open(self.config.metadata_path +
                           sample['img']).convert('RGB'))
            img = img[sample['bbox'][1]:sample['bbox'][3],
                      sample['bbox'][0]:sample['bbox'][2]]
    ...
    def processMesh(self, mesh_path):
        ...
        if self.skip_done and os.path.exists(output_folder +
                                             'uniform_points.sdf'):
            return True
    ...
    def processAllImage(self):
        ...
        ids = [
            int(os.path.basename(file).split('.')[0]) for file in splits
        ...
        with Pool(self.processes) as p:
            r = list(
                tqdm.tqdm(p.imap(self.processImage, samples),
                          total=len(samples)))
        return True
    ...
    def processAllMesh(self):
        ...
        with Pool(self.processes) as p:
            r = list(
                tqdm.tqdm(p.imap(self.processMesh, mesh_path_list),
                          total=len(mesh_path_list)))
        return True
