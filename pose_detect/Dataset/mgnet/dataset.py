...
from pose_detect.Dataset.pix3d.dataset import PIX3D
...
class MGNet_Dataset(PIX3D):
    ...
    def __getitem__(self, index):
        ...
        densities = np.array(
            [max(dists[point_set, 1])**2 for point_set in indices])

        if self.mode == 'train':
            p_ids = np.random.choice(gt_points.shape[0],
                                     self.num_samples_on_each_model,
                                     replace=False)
