...
from pose_detect.Model.resnet import resnet18_full, model_urls
from pose_detect.Model.ldif.encoder.ldif_encoder import LDIFEncoder
...
class ImageEncoder(nn.Module):
    ...
    def loadPretrainedResNetEncoder(self):
        ...
        if pretrained_dict['conv1.weight'].shape != model_dict[
                'conv1.weight'].shape:
            model_dict['conv1.weight'][:, :3,
                                       ...] = pretrained_dict['conv1.weight']
...
class ImageLDIFEncoder(nn.Module):
    ...
    def __init__(self, config, n_classes):
        ...
        self.image_encoder = ImageEncoder(
            self.config['model'].get('bottleneck_size', 2048),
    ...
    def forward(self, image, size_cls):
        ...
        structured_implicit_activations = self.ldif_encoder.forward(
            embedding, size_cls)
        return_dict[
            'structured_implicit_activations'] = structured_implicit_activations
        return return_dict
