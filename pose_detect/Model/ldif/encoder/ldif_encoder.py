...
class LDIFEncoder(nn.Module):
    ...
    def __init__(self, config, n_classes):
        ...
        self.bottleneck_size = self.config['model'].get(
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.bottleneck_size + n_classes, self.bottleneck_size),
            nn.LeakyReLU(0.2, True),
            nn.Linear(self.bottleneck_size, self.bottleneck_size),
            nn.LeakyReLU(0.2, True),
            nn.Linear(self.bottleneck_size,
                      self.element_count * self.element_embedding_length))
    ...
    def forward(self, embedding, size_cls):
        ...
        structured_implicit_activations = torch.reshape(
            ...
            [-1, self.element_count, self.element_embedding_length])
