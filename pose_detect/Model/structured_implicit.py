...
def roll_pitch_yaw_to_rotation_matrices(roll_pitch_yaw):
    ...
    rotation = torch.stack([
        cz * cy, cz * sy * sx - sz * cx, cz * sy * cx + sz * sx, sz * cy,
...
def sample_quadric_surface(quadric, center, samples):
    ...
    half_distance = torch.matmul(quadric,
                                 homogeneous_sample_coords.transpose(-1, -2))
    half_distance = half_distance.transpose(-1, -2)
    algebraic_distance = torch.sum(homogeneous_sample_coords * half_distance,
                                   -1,
                                   keepdim=True)
...
def decode_covariance_roll_pitch_yaw(radius, invert=False):
    ...
    return torch.matmul(torch.matmul(rotation, diag),
...
def sample_cov_bf(center, radius, samples):
    ...
    dist = (x * (c00 * x + c01 * y + c02 * z) + y *
            (c01 * x + c11 * y + c12 * z) + z * (c02 * x + c12 * y + c22 * z))
...
class StructuredImplicit(object):
    ...
    def _generate_symgroup_samples(self, samples):
        ...
        sym_samples *= torch.tensor([-1, 1, 1],
                                    dtype=torch.float32,
                                    device=self.device)
    ...
    def compute_world2local(self):
        ...
        rotation = roll_pitch_yaw_to_rotation_matrices(
