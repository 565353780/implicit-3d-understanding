...
def batch_eval(points, eval_func, num_samples=512 * 512 * 512, batch_size=1):
    ...
    for i in range(num_batches):
        sdf_batch = eval_func(points[:, i * num_samples:i * num_samples +
                                     num_samples])
...
def eval_grid_octree(coords,
    ...
    while reso > 0:
        ...
        sdf[test_mask] = batch_eval(points, eval_func,
...
def reconstruction(structured_implicit,
    ...
    def eval_func(points, structured_implicit):
        ...
        samples = torch.from_numpy(points).to(
            device=structured_implicit.device).float()
        samples = samples.transpose(-1, -2)
        samples = samples.expand(structured_implicit.batch_size, -1, -1)
        pred = structured_implicit.class_at_samples(
            samples, apply_class_transfer=False)[0][..., 0]
        return pred.detach().cpu().numpy()

    sdf = []
    for s in structured_implicit.unbind():
        sdf.append(
            eval_grid_octree(coords,
                             lambda p: eval_func(p, s),
