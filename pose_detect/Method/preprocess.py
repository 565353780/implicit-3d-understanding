...
def sample_pnts_from_obj(data, n_pnts=5000, mode='uniform'):
    ...
    new_pnts = []
    if mode == 'random':
        random_face_ids = np.random.choice(len(data['f']),
                                           n_pnts,
                                           replace=True,
                                           p=distribution)
        random_face_ids, sample_counts = np.unique(random_face_ids,
                                                   return_counts=True)
        ...
        for face_id, sample_count in zip(random_face_ids, sample_counts):
            ...
            weights = np.diff(np.sort(np.vstack([
                ...
                np.random.uniform(0,
                                  1,
                                  size=(len(vid_in_face) - 1, sample_count)),
            ...
            if 'vn' in flags:
                ...
                new_normal = data['vn'][np.array(nid_in_face) -
                                        1].T.dot(weights)
    ...
    for face_idx, face in enumerate(data['f']):
        ...
        dim = len(vid_in_face)
        npnts_dim = (np.math.factorial(dim - 1) * n_pnts_on_face)**(1 /
                                                                    (dim - 1))
        npnts_dim = int(npnts_dim)

        weights = np.stack(
            np.meshgrid(
                *[np.linspace(0, 1, npnts_dim) for _ in range(dim - 1)]), 0)
