...
class LDIFReconLoss(object):
    ...
    def __call__(self, est_data, gt_data, extra_results):
        ldif = est_data['mgn']
        get_phy_loss = self.config.get('loss_weights', {}).get(
            'ldif_phy_loss', 0.0) > 0
        device = gt_data['image'].device

        loss_settings = self.config['model']['mesh_reconstruction'].get(
            'loss_settings', {})
        ...
        surface_optimize = self.config['model']['mesh_reconstruction'][
            'loss_settings'].get('surface_optimize', False)
        sdf_data = {}

        if get_phy_loss:
            bdb3D_form = get_bdb_form_from_corners(
                extra_results['bdb3D_result'])
            structured_implicit = est_data['structured_implicit']
            ldif_center, ldif_coef = est_data['obj_center'], est_data[
                'obj_coef']
            ...
            inside_samples[:, :, 2] *= -1
            obj_samples = recover_points_to_world_sys(bdb3D_form,
                                                      inside_samples,
                                                      ldif_center, ldif_coef)
            max_sample_points = (gt_data['split'][:, 1] -
                                 gt_data['split'][:, 0] -
                ...
                for start, end in gt_data['split']:
                    ...
                    if end > start + 1:
                        ...
                        for obj_ind in range(start, end):
                            ...
                            other_obj_sample = obj_samples[
                                start:end].index_select(
                                    0,
                                    nearest[1:phy_loss_objects + 1]).reshape(
                                        -1, 3)
