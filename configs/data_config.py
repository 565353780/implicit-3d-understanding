...
NYU40CLASSES = [
    ...
    'towel', 'shower_curtain', 'box', 'whiteboard', 'person', 'night_stand',
    'toilet', 'sink', 'lamp', 'bathtub', 'bag', 'otherstructure',
    'otherfurniture', 'otherprop'
]

NYU37_TO_PIX3D_CLS_MAPPING = {
    0: 0,
    1: 0,
    2: 0,
    3: 8,
    ...
    def __init__(self, dataset):
        ...
        if self.dataset == 'sunrgbd':
            ...
            self.__size_avg_path = os.path.join(
                self.metadata_path, 'preprocessed/size_avg_category.pkl')
            self.__layout_avg_file = os.path.join(
                self.metadata_path, 'preprocessed/layout_avg_file.pkl')
    ...
    def __initiate_bins(self):
        ...
        if self.dataset == 'sunrgbd':
            ...
            if os.path.exists(self.__layout_avg_file):
                ...
                raise IOError('No layout average file in %s. Please check.' %
                              (self.__layout_avg_file))
            ...
            bin['layout_ori_bin'] = [[
                np.pi / 4 + i * ORI_LAYOUT_BIN_WIDTH,
                np.pi / 4 + (i + 1) * ORI_LAYOUT_BIN_WIDTH
            ...
            bin['pitch_bin'] = [[
                -60.0 * np.pi / 180 + i * PITCH_WIDTH,
                -60.0 * np.pi / 180 + (i + 1) * PITCH_WIDTH
            ] for i in range(PITCH_NUMBER_BINS)]
            # roll_bin = [[-20 * np.pi/180, 0 * np.pi/180], [0 * np.pi/180, 20 * np.pi/180]]
            bin['roll_bin'] = [[
                -20.0 * np.pi / 180 + i * ROLL_WIDTH,
                -20.0 * np.pi / 180 + (i + 1) * ROLL_WIDTH
            ] for i in range(ROLL_NUMBER_BINS)]
            '''bbox orin, size and centroid bin'''
            # orientation bin
            NUM_ORI_BIN = 6
            ORI_BIN_WIDTH = float(
                2 * np.pi / NUM_ORI_BIN)  # 60 degrees width for each bin.
            # orientation bin ranges from -np.pi to np.pi.
            bin['ori_bin'] = [[(i - NUM_ORI_BIN / 2) * ORI_BIN_WIDTH,
                               (i - NUM_ORI_BIN / 2 + 1) * ORI_BIN_WIDTH]
            ...
            if os.path.exists(self.__size_avg_path):
                ...
                raise IOError(
                    'No object average size file in %s. Please check.' %
                    (self.__size_avg_path))

            bin['avg_size'] = np.vstack(
                [avg_size[key] for key in range(len(avg_size))])
            ...
            DEPTH_WIDTH = 1.0
            # centroid_bin = [0, 6]
            bin['centroid_bin'] = [[i * DEPTH_WIDTH, (i + 1) * DEPTH_WIDTH]
                                   for i in range(NUM_DEPTH_BIN)]
        else:
